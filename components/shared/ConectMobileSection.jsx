...
import { IoCodeSharp, IoRestaurantOutline } from "react-icons/io5";
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-4 md:gap-14 mt-[-60px] md:mt-[-350px]">
        <div className="hidden md:flex flex-col justify-end gap-10 md:gap-28 mx-auto mr-[-2px]">
          {featuresInfoLeft.map((el) => (
            <div
              key={el.id}
              className={`p-2 bg-white rounded-full w-28 h-28 flex flex-col items-center justify-center border border-[#737373] ${hoveredItem === el.id ? "shadow" : ""}`}
        ...
        <div className="grid justify-center grid-cols-3 gap-6 mx-auto mt-8 bg-white md:hidden">
          {featuresInfoAll.map((el) => (
            <div
              key={el.id}
              className={`p-1 bg-white rounded-full w-24 h-24 flex flex-col items-center justify-center border border-[#737373] ${hoveredItem === el.id ? "shadow" : ""}`}
              onMouseEnter={() => hoverItemSelect(el)}
            >
